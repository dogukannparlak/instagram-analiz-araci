...
try:
    ...
    if not usernames:
        print("⚠️  Uyarı: Hiçbir takip edilen bulunamadı!")
        print("💡 İpucu: HTML dosyasının formatı değişmiş olabilir.")
    else:
        print(f"📊 Toplam {len(usernames)} takip edilen bulundu:\n")
        print("=" * 50)
        print("👤 TAKİP EDİLENLER:")
        print("=" * 50)
        for i, username in enumerate(usernames, 1):
            print(f"{i:3d}. {username}")
        print("=" * 50)
        print(f"✅ Analiz tamamlandı! Toplam: {len(usernames)} takip edilen")
